...
class dataloader():
    ...
    def load(self, cropped_date=None, use_optimize=True):
        self.employees = []
        ...
        for row in self.df_flights.iterrows():
            ...
            if cropped_date is None:
                flight = Flight(i, FltNum, DptrDate, DptrTime, DptrStn,ArrvDate, ArrvTime, ArrvStn)
    ...
    def get_FF(self, minCT=45, MinRest=640):
        self.FF = []
        ...
        for i, flight_a in enumerate(self.flights):
            for j, flight_b in enumerate(self.flights):
                count += 1
                if count % 10000 == 0:
                    print("Loading... ({}/{})".format(count, len_FF))
