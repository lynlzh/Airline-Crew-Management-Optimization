...
class RViewer(object):
    def __init__(self, dataloader, data_cls="a"):
        ...
        self.dl = dataloader
        self.mathched_e_f = []
        self.mathched_e_f_dic = {'emp_no':[], 'flg_no':[], 'cls':[]}
        self.mathched_cap_dic = {'emp_no':[], 'flg_no':[]}
        self.matched_fo_dic = {'emp_no':[], 'flg_no':[]}
        self.matched_dh_dic = {'emp_no':[], 'flg_no':[]}
        self.dics = []
    ...
    def generate_dics(self):
        for (em, fl, cls) in self.mathched_e_f:
            self.mathched_e_f_dic['emp_no'].append(em)
            self.mathched_e_f_dic['flg_no'].append(fl)
            self.mathched_e_f_dic['cls'].append(cls)
            if cls == 1:
                self.mathched_cap_dic['emp_no'].append(em)
                self.mathched_cap_dic['flg_no'].append(fl)
            elif cls == 2:
                self.matched_fo_dic['emp_no'].append(em)
                self.matched_fo_dic['flg_no'].append(fl)
            elif cls == 3:
                self.matched_dh_dic['emp_no'].append(em)
                self.matched_dh_dic['flg_no'].append(fl)
    ...
    def get_results_df_b(self):
        ...
        for i in range(len(self.dl.C)):
            ...
            for (fl_no, cls) in fl_list:
                results_dic['emp_no'].append(self.dl.C_dic[i])
                results_dic['leg_no'].append(m)
                results_dic['flg_no'].append(self.dl.F_dic[fl_no].split('/')[0])
                if self.data_cls == "b":
